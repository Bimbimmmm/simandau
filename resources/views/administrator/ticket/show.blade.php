...
<div class="container mx-auto px-4 sm:px-8">
  <div class="py-8">
    <div>
      <h2 class="text-2xl text-center leading-tight">Tiket Dengan Kode : #{{$data->code}}</h2>
    </div>
    <section class=" py-1">
        ...
        <div class="flex bg-blue-100 rounded-lg p-4 mb-4 text-sm text-blue-700" role="alert">
          ...
          <div class="flex bg-yellow-100 rounded-lg p-4 mb-4 text-sm text-yellow-700" role="alert">
            ...
            <div class="flex bg-red-100 rounded-lg p-4 mb-4 text-sm text-red-700" role="alert">
              <svg class="w-5 h-5 inline mr-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
              <div>
                <span class="font-medium">Tiket Sudah Dihapus
              </div>
            </div>
            @endif
            @if($data->is_finished != TRUE)
            <div class="mb-2 md:flex md:items-center">
              <div class="md:w-1/3">
                <button onclick="window.location='{{ url ('/administrator/ticket/finish', array( $idEn )) }}'" class="shadow bg-green-600 hover:bg-green-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded">
            ...
            <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
              ...
              <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
                <form action="{{ url('/administrator/ticket/show/reply', array("$idEn"))}}" method="POST" enctype="multipart/form-data">
                  @csrf
